							...
							<c:otherwise>
							 <option value="${vendorList.vendorId}" ><c:out value="${vendorList.vendorName}"/></option>
									...
									<div class="col-md-3">
										...
										<select id="mrn_status" class="form-control chosen"
											tabindex="6" name="mrn_status" required>
											<c:choose>
    ...
    <i class="fa fa-search" style="font-size:15px"></i>
									...
									<div class="input-group">
										<input type="text" id="myInput"
											...
											onkeyup="myFunction()"
											placeholder="Search By Mrn No or Vendor Name" /> <span
										...
										<table class="table table-advance" id="table1">
											<thead>
												<tr class="bgpink">
													...
													<th class="col-md-2">MRN First Item Name</th>
											...
											<tbody>
												...
												<c:forEach items="${getMrnHeaderList}"
													var="getMrnHeaderList" varStatus="count">
													<tr>
														<td class="col-md-1"><c:out value="${count.index+1}" /></td>
														<td class="col-md-1"><c:out
																value="${getMrnHeaderList.mrnNo}" /></td>
														<td class="col-md-1"><c:out
																value="${getMrnHeaderList.mrnDate}" /></td>
														<td class="col-md-2"><c:forEach items="${typeList}"
																var="typeList">
																<c:choose>
																	<c:when
																		test="${typeList.typeId==getMrnHeaderList.mrnType}">
																		<c:out value="${typeList.typeName}" />
																	</c:when>
																</c:choose>
															</c:forEach></td>
														<td class="col-md-1"><c:out
																value="${getMrnHeaderList.getMrnDetailList[0].itemName}" /></td>


														<td class="col-md-1"><c:choose>
																<c:when test="${getMrnHeaderList.mrnStatus==0}">
																	<c:out value="Gate Inward Done /Stores MRN Pending" />
																</c:when>
																<c:when test="${getMrnHeaderList.mrnStatus==2}">
																	<c:out
																		value="Stores MRN Done/Director Approval Pending" />
																</c:when>
																<c:when test="${getMrnHeaderList.mrnStatus==2}">
																	<c:out
																		value="Stores MRN Done/Director Approval Pending" />
																</c:when>
																<c:when test="${getMrnHeaderList.mrnStatus==3}">
																	<c:out value="Director Approval Done" />
																</c:when>
																<c:when test="${getMrnHeaderList.mrnStatus==4}">
																	<c:out value="Director Approval Done" />
																</c:when>
																<c:when test="${getMrnHeaderList.mrnStatus==1}">
